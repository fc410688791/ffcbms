...
class DefaultAcsClient implements IAcsClient
{
    ...
    private function doActionImpl($request, $iSigner = null, $credential = null, $autoRetry = true, $maxRetryNumber = 3)
    {
        ...
        if (null != $request->getLocationServiceCode()) {
            $domain =
                $this->locationService->findProductDomain('cn-shanghai', // $request->getRegionId(), 由于$request->getRegionId() == 'cn-hangzhou' 时获取不到$domain, 而下面获取到的$domain = EndpointProvider::findProductDomain($request->getRegionId(), $request->getProduct()) = 'iot.aliyuncs.com'; 发送的API请求为：服务不可用. 所以这里固定为：’cn-shanghai‘; #leeprince_20190515
        ...
        /*debugLog('阿里云物联网平台API请求-getRegionId:'.$request->getRegionId());
        debugLog('阿里云物联网平台API请求-getLocationServiceCode:'.$request->getLocationServiceCode());
        debugLog('阿里云物联网平台API请求-getLocationEndpointType:'.$request->getLocationEndpointType());
        debugLog('阿里云物联网平台API请求-getProduct:'.$request->getProduct());
        debugLog('阿里云物联网平台API请求-domain:'.$domain);
        debugLog('阿里云物联网平台API请求-URL:'.$requestUrl);
        debugLog('阿里云物联网平台API请求-getMethod:'.$request->getMethod());
        debugLog('阿里云物联网平台API请求URL-getDomainParameter:'.json_encode($request->getDomainParameter()));
        // prt_exit($requestUrl);
        // exit(0);*/
