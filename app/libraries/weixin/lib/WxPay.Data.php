 ...
 *
 */
class WxPayDataBase extends Weixin_pay
{
	...
	**/
	...
	{
	    $pu_key = openssl_pkey_get_public(file_get_contents(Weixin_pay::$_config['PUBLIC_PATH']));  //读取公钥内容
	    $encryptedBlock = '';
	    $encrypted = '';
	...
	**/
	...
	public function MakeSign()
	{
		...
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".Weixin_pay::$_config['KEY'];
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayMchPay extends WxPayDataBase{
    ...
    {
        $this->values["partner_trade_no"] = $PartnerTradeNo;
    }
    /**
     * 金额
     * @param type $amount
    ...
    {
         $this->values["check_name"] = $checkname;
    }
    /**
     * NO_CHECK：不校验真实姓名 
       FORCE_CHECK：强校验真实姓名
    ...
    public function SetMch_id($mchid)
    {
 ...
 *
 */
class WxPayBank extends WxPayDataBase
{
    ...
    /**
     * 收款方银行卡号
     * @param type $enc_bank_no
     */
    public function SetEncBankNo($enc_bank_no)
    {
        $enc_bank_no = self::rsa_encrypt($enc_bank_no);
        $this->values["enc_bank_no"] = $enc_bank_no;
    }
    
    /**
     * 收款方用户名
     * @param type $enc_true_name
     */
    public function SetEncTrueName($enc_true_name)
    {
        $enc_true_name = self::rsa_encrypt($enc_true_name);
        $this->values["enc_true_name"] = $enc_true_name;
    }
    
    /**
     * 收款方开户行
     * @param type $bank_code
     */
    public function SetBankCode($bank_code)
    {
        $this->values["bank_code"] = $bank_code;
    }
}
 ...
 *
 */
class WxPayGetPublicKey extends WxPayDataBase
{
    public function SetMchId($mchid)
    {
        $this->values["mch_id"] = $mchid;
