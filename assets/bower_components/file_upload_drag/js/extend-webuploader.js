jQuery(function(){
    ...
    if( !window.webuploader ){
        console.log("璇烽厤缃ソwindow.webuploader");
        $('#dndArea p').html('璇烽厤缃ソwindow.webuploader');
        return false;
    }
    if( !window.webuploader.config || !window.webuploader.config.wrapId){
        console.log("璇烽厤缃ソwindow.webuploader.config.wrapId");
        $('#dndArea p').html('璇烽厤缃ソwindow.webuploader.config.wrapId');
        return false;
    }
    if( !window.webuploader.uploadUrl ){
        console.log("璇烽厤缃ソwindow.webuploader.uploadUrl");
        $('#dndArea p').html('璇烽厤缃ソwindow.webuploader.uploadUrl');
        return false;
    }
    if( !window.webuploader.updateUrl ){
        console.log("璇烽厤缃ソwindow.webuploader.updateUrl");
        $('#dndArea p').html('璇烽厤缃ソwindow.webuploader.updateUrl');
        return false;
    }
    if( !window.webuploader.removeUrl ){
        console.log("璇烽厤缃ソwindow.webuploader.removeUrl");
        $('#dndArea p').html('璇烽厤缃ソwindow.webuploader.removeUrl');
        return false;
    }
    var disX = 0;
    var disY = 0;
    ...
    function updateStatus(){
        var text = '', stats;
        if( state == 'ready'){
            text = '閫変腑'+fileCount + '寮犲浘鐗囷紝鍏�'+ WebUploader.formatSize( fileSize ) +'.';
        } else if( state == 'confirm'){
            stats = uploader.getStats();
            if( stats.uploadFailNum ){
                text = '宸叉垚鍔熶笂浼�'+stats.successNum+'寮犵収鐗�'+stats.uploadFailNum +'寮犵収鐗囦笂浼犲け璐�,<a class="retry" href="#">閲嶆柊涓婁紶</a>澶辫触鍥剧墖鎴�<a class="ignore" href="#">蹇界暐</a>';
            }
        } else {
            stats = uploader.getStats();
            text = '鍏�' + fileCount +'寮�('+WebUploader.formatSize(fileSize)+')锛屽凡涓婁紶'+stats.successNum+'寮�';
            if( stats.uploadFailNum){
                text += ',澶辫触'+ stats.uploadFailNum +'寮�';
            }
        }
        $info.html(text);
    }
