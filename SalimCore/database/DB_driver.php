...
abstract class DB_driver {
	...
	{
		$this->dbProxy($this->is_write_type($sql), $this->db_force_master);
		if ( ! $this->conn_id)
		{
			$this->initialize();
		}
	 ...
	 * [dbProxy 数据库代理是否读写]
	 *
	 * @Author leeprince:2019-03-20T12:07:06+0800
	 * @param  [type]                             $isWriterType  [description]
	 * @param  [type]                             $isForceMaster [description]
	 * @return [type]                                            [description]
	 */
	public function dbProxy($isWriterType, $isForceMaster)
	{
        $proxy_setting = load_db_proxy_setting($this->group_name, $isWriterType, $isForceMaster);
		if(is_array($proxy_setting) && ! empty($proxy_setting)) {
			...
			$CI = & get_instance();
			if(isset($CI->$proxy_conn_id) && is_object($CI->$proxy_conn_id)) {
			...
			if ($isWriterType || $isForceMaster) {
				$this->setMasterConnId();
			}
